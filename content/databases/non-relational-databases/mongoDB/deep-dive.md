...
Creating a document inserts new data into a collection.
...
When a document does not include an `_id` field, MongoDB automatically generates an `ObjectId` before storing it.

Multiple documents can also be inserted together.
...
Reading retrieves documents that match a query.
...
When only a single document is expected, `findOne()` is commonly used.
...
The MongoDB query language allows filters, projections, sorting, and many other operations, which are explored in the next chapter.

## Update

Updating modifies existing documents without replacing the entire document.
...
MongoDB also supports updating multiple matching documents or replacing an existing document completely.

## Delete

Deleting permanently removes documents from a collection.
...
When multiple documents match a filter, they can be removed together using `deleteMany()`.
